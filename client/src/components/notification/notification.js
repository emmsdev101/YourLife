...
function NotificationItem({ unread, notification, setRenderHeader, setViewNotification, openProfile }) {
  const my_api =
    ...
    if(!seen){
      if(notification.type === 'comment'){
        axios({
          ...
          data:{comment_id:notification.comment_id, notification_id:notification.notification_id}
        });
      }
      else{
        axios({
 ...
 if (notification.type === "comment") {
    ...
    return (
      <div
        className={
          !seen ? "notification-div unread" : "notification-div"
        }
